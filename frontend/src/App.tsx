...
import { StudentMaterialsView, StudentPracticeView, TeacherEvidenceView, TeacherPublishView } from "./components/StudentSection";
...
export default function App() {
  ...
  const [screen, setScreen] = useState<"request" | "generating" | "results" | "audit" | "coverage" | "student-materials" | "student-practice" | "student-evidence">("request");
  const [practiceReleaseId, setPracticeReleaseId] = useState<string | null>(null);
  ...
  return <main className="min-h-screen bg-[#f7f4ed]"><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
    <header className="no-print flex flex-wrap items-center gap-3"><ClaraMark state={headerMarkState} size="sm" /><div><p className="font-serif text-xl font-semibold text-stone-900">Clara</p><p className="text-xs text-stone-500">Compañera de planificación docente</p></div>{phase === "signed_in" && role === "student" && <button onClick={() => setScreen("student-materials")} className="ml-auto rounded-lg bg-[#edf5f0] px-3 py-2 text-sm font-semibold text-[#195b4e]">Mis materiales</button>}{phase === "signed_in" && role === "teacher" && <button onClick={() => setScreen("student-evidence")} className="ml-auto rounded-lg bg-[#edf5f0] px-3 py-2 text-sm font-semibold text-[#195b4e]">Evidencia del curso</button>}<AuthPanel />{isMockMode && <details className="text-xs text-stone-500"><summary className="cursor-pointer rounded-full border border-stone-200 bg-white px-3 py-1.5 font-semibold marker:content-none">Vista previa</summary><label className="mt-2 flex items-center gap-2 rounded-lg bg-white p-2 shadow-sm">Velocidad <select value={mockSpeed} onChange={(event) => setMockSpeed(event.target.value as MockSpeed)} className="rounded border border-stone-300 bg-white px-1.5 py-1"><option value="slow">Lenta</option><option value="normal">Normal</option><option value="fast">Rápida</option></select></label></details>}</header>
    {phase === "expired" && <p className="no-print mt-5 rounded-xl border border-amber-200 bg-[#fff5df] p-4 text-sm text-amber-950">Tu sesión venció o fue rechazada. Inicia sesión nuevamente para continuar en la sección de estudiantes.</p>}
    ...
    {screen === "student-materials" && phase === "signed_in" && role === "student" && <StudentMaterialsView onBack={() => setScreen("request")} onOpen={(releaseId) => { setPracticeReleaseId(releaseId); setScreen("student-practice"); }} />}
    {screen === "student-practice" && phase === "signed_in" && role === "student" && practiceReleaseId && <StudentPracticeView releaseId={practiceReleaseId} onBack={() => setScreen("student-materials")} />}
    {screen === "student-evidence" && phase === "signed_in" && role === "teacher" && <TeacherEvidenceView onBack={() => setScreen(plan ? "results" : "request")} />}
    {screen === "results" && plan && guide && assessment && review && <>{phase === "signed_in" && role === "teacher" && <TeacherPublishView plan={plan} guide={guide} assessment={assessment} review={review} />}<TeachingPackResults plan={plan} guide={guide} assessment={assessment} review={review} materials={materials} onPlanChange={changedPlan} onGuideChange={changedGuide} onAssessmentChange={changedAssessment} onMaterialsChange={changedMaterials} onGenerateMaterials={createMaterials} materialsBusy={materialsBusy} onReviewEdits={reviewEdits} editReviewBusy={editReviewBusy} editReviewError={editReviewError} editReviewStatus={editReviewStatus} hasEdits={hasEdits} onReplay={isMockMode ? () => { setShowGenerator(true); setScreen("request"); } : undefined} /></>}
  </div></main>;
}
